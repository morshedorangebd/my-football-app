...
export default async function TeamsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-zinc-900 dark:text-white">Teams</h1>

      {teams.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 sm:gap-4">
          {teams.map((team) => (
            <TeamBadge key={team.id} id={team.id} name={team.name} logo={team.logo} size="lg" responsive />
          ))}
        </div>
      ) : (
